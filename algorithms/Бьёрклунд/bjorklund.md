...
Следующая лемма показывает, как исчезают негамильтоновые покрытия циклами, что также означает отсутствие ложных срабатываний в алгоритме.
...
**Утверждение.** Определитель матрицы $\textbf{A}$ размера $n \times n$ над кольцом характеристики два совпадает с _перманентом_, то есть:

$$
\mathrm{det}(\textbf{A}) = \mathrm{per}(\textbf{A}) = \sum_{\sigma \ : \ [n] \rightarrow [n]} \prod_{i = 1}^{n} \textbf{A}_{i, \sigma}(i),
$$
...
Перманент можно интерпретировать как сумму взвешенных покрытий циклами направленного графа. Иначе говоря, пусть $D = (V, A)$ - направленный граф с весами $w \ : \ A \rightarrow R$, определим матрицу $\textbf{A}$ размера $|V| \times |V|$ с строками и столбцами в качестве вершин $V$
...
$$
\begin{align}
\mathrm{per}(\textbf{A}) = \sum_{C \in cc(D)} \prod_{a \in C} w(a)
\end{align}
$$
...
$$
\begin{align}
p(f, r) = \sum_{Y \subseteq L} \mathrm{det} \left(\sum_{Z \subseteq Y} r^{|Z|} M_f(Z)\right)
\end{align}
$$

Для лучшего понимания рассмотрим подробнее внутреннюю матрицу, обозначенную за $T_f(Z)$. Пронумеруем для удобства все подмножества $Z \subseteq Y$ в порядке позрастания по их мощности, то есть $|Z_1| \leq |Z_2| \leq \ldots \leq |Z_{2^{|Y|}}|$. Тогда элементы матрицы будут иметь вид полинома

$$
\begin{align*}
T_f(Z)_{i, j} &= r^{|Z_{2^{|Y|}}|} f(ij, Z_{2^{|Y|}}) \\
...
>
> $$
> p(f, r) = \sum_{Y \subseteq L} \sum_{C \in cc(D)} \sum_{q \ : \ C \rightarrow 2^Y \text{ \\ } \\{\emptyset\\}} \prod_{a \in C} r^{|q(a)|} f(a, q(a))
> $$
>
...
> p(f, r) =
> \sum_{С \in cc(D)}
> \sum_{q \ : \ C \rightarrow 2^L \text{ \\ } \\{\emptyset\\}}
> {\sum_{\substack{
...
>
> Для функций $q \ : \ C \rightarrow 2^L \text{ \\ } \\{\emptyset\\}$ таких, что $\bigcup_{a \in C} q(a) \subset L$, то есть чьё объединение при данном покрытии не покрывает всё множество $L$, получаем, что последнее суммирование происходит _чётное_ количетсво раз с одним и тем же слагаемым (таких слагаемых $2^{|L \text{ \ } \bigcup_{a \in C} q(a)|}$ штук, так как количетсво подмножеств $Y$, включающих подмножество $\bigcup_{a \in C} q(a)$ равно количеству подмножеств $L \text{ \ } \bigcup_{a \in C} q(a)$ - чётное число). По итогу получаем
>
> $$
> p(f, r) =
> \sum_{C \in cc(D)}
> \sum_{\substack{q : C \to 2^L \text{ \ } \\{\emptyset\\}\\\bigcup_{a \in C} q(a) = L}} r^{\sum{a \in C} |q(a)|}
...
> $$
>
> Заметим, что $q$ отображает в подмножетсва, покрывающих $L$. Обращая функцию $q$ получаем исходное определение суммы маркированных покрытий $(1)$.
> $$
