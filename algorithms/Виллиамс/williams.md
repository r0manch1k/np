...
популярных примеров является $MAX\text{-}2\text{-}SAT$.

**Определение.** Задача $MAX\text{-}2\text{-}SAT$ яляется задачей определения максимального количества клозов в $2\text{-}SAT$ формуле, которые могут выполниться после присваивания значений переменным.

В последнее время всё больше проявляется теоретический интерес в решении $MAX\text{-}2\text{-}SAT$ за $O((2 - \epsilon)^n)$ шагов при любыx входных данных для какого-то $\epsilon > 0$. В отличии от таких задач как $Vertex \ Cover$ и $k\text{-}SAT$, где для улучшения тривиальных границ по времени достаточно было провести сложный анализ методов «разветвления и ограничения» (или присваивания значений), $MAX-SAT$ оказался сложнее. Некоторые из предыдущих работ смогли показать либо границу $(2 - \epsilon)^m$, где $m$ - количество клозов, либо аппроксимацию $(2 - \epsilon)^n$ (но $\epsilon \rightarrow 0$). Конечно, количество клозов может в общем случае быть куда больше, чем количество переменных, поэтому граница $(2 - \epsilon)^m$ даст прирост по скорости по сравнению с тривиальной границе только в некоторых частных случаях.
...
На момент написания статьи лучшая граница по времени для $MAX\text{-}2\text{-}SAT$ - это $O^*(2^{m/5})$ (т.е. для $m/n > 5$ она не лучше $2^n$). Частичный ответ на вопрос о существовании $(2 - \epsilon)^n$ алгоритма для $MAX\text{-}SAT$ был дан автором статьи (Райаном Виллиамсом), который доказал, что _разряжённый_ экземпляр задачи $MAX\text{-}k\text{-}SAT$ может быть решён за $(2 - \epsilon)^n$. То есть, для любых констант $k$ и $\Delta$ существует константа $c_{k,\Delta}$ такая, что $MAX\text{-}k\text{-}SAT$ с максимум $\Delta n$ клозами решается за $(c_{k,\Delta})^n$ шагов (однако, $c_{k,\Delta} \rightarrow 2$ при $\Delta \rightarrow \infty$ или $k \rightarrow \infty$). Также показано, что когда каждая переменная появляется в формуле максимум 3 раза, $MAX\text{-}2\text{-}SAT$ становится $NP$-Полной. но имеет $O(n3^{n/2})$ алгоритм. Все эти результаты остаются далеки от реального улучшения экспоненциального алгоритма, поэтому некоторые исследователи признают нахождение $(2 - \epsilon)^n$ алгоритма для $MAX\text{-}2\text{-}SAT$ (и/или $MAX\text{-}CUT$) открытой проблемой.
...
**Определение.** Задача $MAX\text{-}CSP$ яляется задачей определения такого присваивания значений переменным $x_1, x_2, \ldots, x_n$, при котором достигается максимальное количество выполняющихся ограничений $c$.
...
Если существует $i$ такое, что $A(G_r)^3[i, i] \neq 0$, то чтобы найти $3r$-клику из графа $G$, перебираем все соединённые с $i$-й кликой $j$-клики и наконец ищем для $j$-клики соединённую с ней $k$-клику. Таким образом найдём исходную клику за $O(n^{2r})$. Также ниже будет приведён ещё один способ восстановления клики за дополнительный фактор $O(n)$.

## 4. Алгоритм для $COUNT\text{-}2\text{-}CSP$

Сначала формально опишем нашу задачу, а именно $COUNT\text{-}2\text{-}CSP$.
...
**Теорема.** Пусть $k(n) \geq 3$ является монотонной невозрастающей функцией. Тогда задачу $COUNT\text{-}2\text{-}CSP$ можно решить за $O\left(\binom{N - \binom{k(n)}{2} - 1}{\binom{k(n)}{2} - 1}[k(n) d^{n / k(n)}]^{\kappa(k(n))}\right)$, где $m$ - количество ограничений, $n$ - количество переменных и $d$ - размер области определения переменных.

**Следствие.** Задачи $MAX\text{-}2\text{-}SAT$ и $MAX\text{-}CUT$ могут быть решены за время $O(m^3 1.729^n)$.
...
## 5. Релизация алгоритма для $MAX\text{-}CUT$

Теперь на основе приведённого алгоритма решим задачу $MAX\text{-}CUT$ на языке **Go**.

**Определение.** Задача $MAX\text{-}CUT$ заключается в нахождении такого разбиения множества вершин неориентированного графа $G = (V, E)$ на два непересекающихся подмножества $V_1$ и $V_2$, при котором максимизируется число рёбер $\\{u,v\\} \in E$, для которых $u \in V_1$ и $v \in V_2$.
