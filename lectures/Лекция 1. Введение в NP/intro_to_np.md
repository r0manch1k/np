...
Временная сложность вычислительных задач представляет большой интерес как для теоретических, так и для практических приложений. В частности, многие проблемы, такие как $SAT$ (задача выполнимости булевых формул) и $TSP$ (задача коммивояжёра), принадлежат к большому классу, известному как $NP$.
...
В 1969 году C. Кук расширил исследование Тьюринга о том, что можно и что нельзя вычислить, и смог отделить те задачи, которые могут
быть эффективно решены с помощью компьютера, от тех, которые в теории могут быть решены, но на практике их решение требует так много времени, что компьютеры бесполезны для решения почти всех из них, кроме очень небольшого количества их частных случаев. Последний класс проблем называется неразрешимыми
или $NP$-сложными.
...
Теорию неразрешимости принято строить не на программах на Си или другом языке, а на очень простой модели компьютера - машине Тьюринга. Одно из преимуществ машины Тьюринга перед такими программами заключается в том, что машина Тьюринга достаточно проста, чтобы мы могли точно представить ее конфигурацию, используя простые обозначения. В то время, как описания состояний в программах слишком сложны, чтобы мы могли сделать понятные формальные доказательства.
...
> **Пример.** Расмотрим задачу о гамильтоновом пути (путь содержит все вершиныи и ровно один раз). Обозначим её как язык $HAMPATH = \\{\left<G, s, t\right>: G \text{ - направленный граф с гамильтоновым путём от } s \text{ до } t\\}$. Тогда сертификат $y$ будет представлять собой непосредственно гамильтонов путь от вершины $s$ до вершины $t$. Верификатор $R(\left<G, s, t\right>, y)$ проверяет, является ли $y$ подграфом $G$ и проходит ли путь все вершины графа $G$ ровно один раз. Если да, то решение $x = \left<G, s, t\right>$ принимается и $(\left<G, s, t\right>, y) \in R$ (или $\left<G, s, t\right> \in HAMPATH$), в обратном случае - отверагется. В рассмотренном случае верификатор $R$ может за полиномиальное время проверить, что входные данные были введены на том языке, на котором ему выдан сертификат.
...
> **Пример.** Всем известная задача $SAT$ на разрешимость булевой формулы имеет функциональный аналог $FSAT$, который выглядит следующим образом: _Дана булева формула_ $\phi$ _с переменными_ $x_1, x_2, ..., x_n$. _Найти такие_ $x_i \in \\{TRUE, FALSE\\}$, _чтобы_ $\phi$ _была_ $TRUE$ _или принять решение о том, что не существует такого набора переменных._ В этом случае $A$ состоит из пар закодированных булевых формул и удовлетворяющих наборов переменных.
...
> N = &\text{ На вход } x: \\
> & 1. \text{ Вычисляем } f(x) \\
> & 2. \text{ Запускаем } \mathfrak{M} \text{ с } f(x) \text{ на входе и выдаём всё, что выдаёт } \mathfrak{M} \\
> \end{align*}
> $$
>
> По условию $x \in A$ всякий раз, когда $f(x) \in B$, потому что $f$ - это сведение от $A$ к $B$. То есть $\mathfrak{M}$ принимает $f(x)$ всякий раз, когда $x \in A$. Более того, $N$ выполняется за полиномиальное время, потому что каждый из двух его этапов выполняется за полиномиальное время, а сумма двух многочленов является многочленом.

Примеры подобных сведений приведены в [Лекции 2. Сведение SAT](./../Лекция%202.%20Сведение%20SAT/lecture.md).
...
**Определение.** Задача $A$ является $NP$-Полной, если $A \in NP$ и она $NP$-Трудная.

**Теорема (Транзитивность).** Если $B$ является $NP$-Полной задачей и $B \leq_P C$, где $C \in NP$, то $C$ является $NP$-Полной.

> **Доказательство.** Нам нужно показать, что каждый язык $L$ за $NP$ полиномиальное время сводится к $C$. Мы знаем, что существует полиномиальное сведение $L$ к $B$ за время, не превосходящее $p(n)$. То есть, строка $w$ в $L$ длины $n$ отображается в строку $x$ в $B$ не более, чем за $p(n)$.
>
...
Это, наверное, самая важная теорема в этой лекции.
...
Опустим формальное определение оракула, которое требует введение понятия оракул-машины Тьюринга.

## 5. Пространственная сложность

В этом разделе мы рассмотрим сложность вычислительных задач с точки зрения объема памяти, который для них требуется. _Время_ и _память_ - два наиболее важных фактора, которые мы учитываем при поиске практических решений. Пространственная сложность имеет много общих черт с временной сложностью и служит дополнительным способом классификации задач в соответствии с их вычислительной сложностью.

Как и в случае с временной сложностью, нам нужно выбрать модель для измерения памяти, используемой алгоритмом. Мы продолжаем использовать модель машины Тьюринга по той же причине, по которой мы использовали её для измерения времени. Машины Тьюринга математически просты и достаточно близки к реальным компьютерам, чтобы выдавать значимые результаты.
...
**Теорема (У. Савич).** Для любой функции $f: \mathbb{N} \longrightarrow \mathbb{R}^+$, где $f(n) \geq n$, $NSPACE(f(n)) \subseteq SPACE(f^2(n))$. То есть любая недетерминированная машиина Тьюринга, использующая $f(n)$ памяти может быть представлена в виде детерминированной машины Тьюринга, использующей только $f^2(n)$ памяти.
