# Лекция 1: Введение в NP-Полные задачи
...
Временная сложность вычислительных задач представляет большой интерес как для теоретических, так и для практических приложений. В частности, многие проблемы, представляющие интерес, такие как _SAT_ (задача выполнимости булевых формул) и _TSP_ (задача коммивояжёра), принадлежат к большому классу, известному как $NP$.
...
В задаче разрешимости ответом может быть либо _да_, либо _нет_. Аналогично, задача разрешимости может быть представлена в виде набора $A$ возможных входных данных, на который ответ будет _да_ (т.е. $A \subseteq \Sigma^*$, где $\Sigma^*$ - слова, составленные из букв алфавита $\Sigma$). $A$ также называют _языком_.
...
**Определение (Р. Карп).** Задача разрешимости $A$ является _полиномиально сводимой_ к другой задаче разрешимости $B$, если существует вычислимая за полиномиальное время функция $f$, определённая на всевозможных входных данных для $A$ так, что $x \in A \Longleftrightarrow f(x) \in B$. Обозначается $A \leq_P B$.
...
Почему бы нам не определить понятие сводимости к полиномиальному пространству и не использовать его вместо сводимости к полиномиальному времени? Чтобы понять
ответ на этот важный вопрос, рассмотрим, в первую очередь, нашу мотивацию для определения полных задач.

Полные задачи важны, потому что они являются примерами самых
сложных задач в классе сложности. Полная задача является самой сложной, потому что любая другая задача в классе легко сводится к ней. Таким образом, если мы найдем
простой способ решить задачу в целом, мы сможем легко решить все остальные задачи в
классе. Правило таково: всякий раз, когда мы определяем полные задачи для класса сложности, модель сокращения должна быть более ограниченной, чем модель, используемая для
определения самого класса.
